...
struct Material
{
    ...
    explicit Material(const std::string &name)
            : mName(name)
            , mDiffuse            ( float( 0.6 ), float( 0.6 ), float( 0.6 ) ) // FIX ... we won't want object to be black
            , mSpecularExponent   ( float( 0.0 ) )
            , mShininessStrength  ( float( 1.0 ) )
            , mShading(Discreet3DS::Gouraud)
            , mTransparency       ( float( 1.0 ) )
            , mBumpHeight         ( float( 1.0 ) )
            , mTwoSided           (false)
    {
    }
...
struct Mesh : public MeshWithSmoothingGroups<Face>
{
    //! Default constructor has been deleted
    Mesh() = delete;
