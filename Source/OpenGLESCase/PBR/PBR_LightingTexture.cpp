...
void PBR_LightingTexture::CreateScence(Context *context, int width, int height)
{
    ...
    float pi = 3.1415926f;
    //相机位置数据
    PRBGraph_.eyePos = glm::vec3(0,2,3);
    //组件mvp矩阵
    PRBGraph_.modelMatrix = glm::translate(glm::vec3(0,-0.5,0))*glm::rotate(0.0f,glm::vec3(0,1,0))* glm::rotate(-pi/2.0f,glm::vec3(1,0,0))*glm::scale(glm::vec3(5.0f,5.0f,5.0f));//pi/2.0f
    ...
    //光照位置
    PRBGraph_.lightPos[0] = glm::vec3(-3.0f, 0.0f, 0.0f);
    PRBGraph_.lightPos[1] = glm::vec3(-1.0f, 0.0f, 0.0f);
    PRBGraph_.lightPos[2] = glm::vec3 (1.0f, 0.0f, 0.0f);
    PRBGraph_.lightPos[3] = glm::vec3 (3.0f, 0.0f, 0.0f);
    //光照颜色
    PRBGraph_.lightColor[0] = glm::vec3(150.0f);
    PRBGraph_.lightColor[1] = glm::vec3(150.0f);
    PRBGraph_.lightColor[2] = glm::vec3(150.0f);
    PRBGraph_.lightColor[3] = glm::vec3(150.0f);
...
void PBR_LightingTexture::RenderScence()
{
    ...
    glVertexAttribPointer(PRBGraph_.positionHandle, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(GL_FLOAT), Sphere_->getData().data());
    glVertexAttribPointer(PRBGraph_.normalHandle, 3, GL_FLOAT, GL_FALSE,8 * sizeof(GL_FLOAT), (reinterpret_cast<char*>(Sphere_->getData().data())+3*sizeof(GL_FLOAT)));
    glVertexAttribPointer(PRBGraph_.texCoordsHandle, 2, GL_FLOAT, GL_FALSE, 8 * sizeof(GL_FLOAT),(reinterpret_cast<char*>(Sphere_->getData().data())+6*sizeof(GL_FLOAT)));
    ...
    glDrawElements(GL_TRIANGLES, 6,GL_UNSIGNED_INT,Sphere_->getIndex().data());
